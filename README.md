...
```json
{
  ...
  "backend_command": "python C:\\ps_jobs\\job_0001\\orchestrator.py",
  "backend_output_path": "C:\\ps_jobs\\job_0001\\out\\result.png"
}
```

**Параметры:**
- `photoshop_path`: Полный путь к исполняемому файлу Photoshop (устаревший параметр)
...
- `backend_command`: Команда для запуска внешнего бэкенда (например, Python-скрипта)
- `backend_output_path`: Путь к готовому PNG-файлу, который генерирует внешний бэкенд

## Описание работы

1. При получении запроса сервер генерирует уникальный идентификатор задачи (UUID)
2. Создает папку для задачи в `work_dir`
3. Запускает внешний бэкенд, указанный в `backend_command`
4. Ждет завершения выполнения бэкенда или таймаута (max_execution_seconds)
5. Проверяет наличие выходного файла по пути `backend_output_path`
6. Если файл найден — копирует его в папку задачи и возвращает как PNG
7. Если файл не найден — возвращает JSON-сообщение с ошибкой и логами бэкенда
